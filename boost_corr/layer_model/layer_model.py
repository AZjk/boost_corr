...
class CorrLayer(nn.Module):
    def __init__(self, queue_size=1024, num_channels=512, delay_list=(1, 2, 3),
        ...
        self.register_buffer('queue', torch.zeros(self.total_size, num_channels))

        self.bound = (self.reserve_size, self.total_size)

        self.register_buffer('corr_result',
                             torch.zeros(len(delay_list), num_channels))
        ...
        self.corr_length = 0
        self.register_buffer('signal_accumulated', torch.zeros(num_channels))
        self.avg_length = 0
        self.use_ipf = use_ipf
    
    def reset(self):
        self.queue._zero()
        self.signal_accumulated._zero()
        self.ptr = self.reserve_size
        self.corr_length = 0
        self.avg_length = 0
    ...
    def get_signal_sum(self):
        with torch.no_grad():
            signal = self.signal_accumulated.float().cpu().numpy()
        return signal
    ...
    def calc_g2(self):
        ...
        with torch.no_grad():
            up = self.corr_result.float().cpu().numpy() / eff_len.reshape(-1, 1)
            dn = self.signal_accumulated.float().cpu().numpy() / avg_length 
    ...
    def get_result(self):
        ...
        with torch.no_grad():
            G2 = self.corr_result.float().cpu().numpy() / eff_len.reshape(-1, 1)
            IP_t = self.signal_accumulated.float().cpu().numpy() / avg_length
    ...
    def flush(self, hard_flush=False):
        ...
        for n, delay in enumerate(self.delay_list):
            ...
            # self.corr_result[n] += torch.sum(self.queue[sl_a] \
            # * self.queue[sl_b], axis=0)
        ...
        if True:
            ...
            self.queue[target_sl] = self.queue[source_sl].clone()
        # else:
...
def test_01():
    ...
    cl_2 = CorrLayer(queue_size // 2, num_channels, delay_list)

    model = torch.nn.Sequential(cl_1, cl_2)
    model.to(device)
    ...
    with torch.no_grad():
        for n in trange(num_batch):
            # x = torch.rand(batch_size, num_channels, device=device)
            x = torch.ones(batch_size, num_channels, device=device)
            model.forward(x)
...
def test_02_hard_flush():
    ...
    for n in trange(num_batch):
        ...
        with torch.no_grad():
            # model.forward(x, hard_flush=hard_flush)
            x2 = cl_1.forward(x, hard_flush)
            x3 = cl_2.forward(x2, hard_flush)
