...
class TwotimeCorrelator:
    def __init__(
        ...
        valid_size = self.dq_slc[-1].stop
        assert valid_size == mask_crop.numel(), f"mask_crop must equal to valid_size"

        num_dq = len(self.dq_slc)
        if self.method == "normal":
            shape = (frame_num, valid_size)
        elif self.method == "window":
            shape = (window * 2, valid_size)
            self.c2 = torch.zeros((num_dq, frame_num - window + 1, window), device=device)
            self.c2_ptr = 0
    ...
    def process_dataset(self, ds, verbose=True, use_loader=False, num_workers=16):
        if not use_loader:
            ...
            dl = DataLoader(ds, batch_size=None, pin_memory=pin_memory, num_workers=num_workers, prefetch_factor=4)
    ...
    def compute_average(self, num_part=10):
        pixel_sum_par = torch.zeros(num_part, self.pixel_num, dtype=torch.float32, device=self.device)
    ...
    def get_twotime_generator(self, **kwargs):
        ...
        results = {"c2_g2": self.g2full, "c2_g2_segments": self.g2partial, "processed_bins": self.dq_idx}
    ...
    def calc_normal_twotime(self, num_partials=5):
        ...
        for n, _ in enumerate(self.dq_idx):
            ...
            for idx in range(num_partials):
                ...
                temp = compute_diagonal_average(diag_mat_1d_par, c2[sl, sl].reshape(-1))
