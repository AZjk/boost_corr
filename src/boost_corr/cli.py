"""
Command-line interface for the Boost Correlation Analysis Tool.

This module provides functions to parse command line arguments and execute
the correlation analysis algorithms for multitau and twotime calculations.
"""
...
from boost_corr.gpu.gpu_corr_multitau import solve_multitau
from boost_corr.gpu.gpu_corr_twotime import solve_twotime
from boost_corr.gpu.gpu_record import get_gpu, release_gpu
...
def convert_to_list(input_str: str) -> Optional[List[int]]:
    """
    Convert a string to a list of integers.

    Parameters:
        input_str (str): A comma-separated string of numbers. If the string is 'all', returns None.

    Returns:
        Optional[List[int]]: A list of integers parsed from the string, or None if input is 'all'.
    """
    if input_str == "all":
        return None
...
def main() -> int:
    """Main entry function for boost_corr CLI.

    This function parses command line arguments, executes the appropriate correlation analysis,
    and returns an exit code indicating success (0) or failure (non-zero).
