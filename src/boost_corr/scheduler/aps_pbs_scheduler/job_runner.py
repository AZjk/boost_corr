...
def _submit_pbs_scripts(job_paths: list):
    ...
    The submit server shares the same filesystem, so scripts are referenced by
    their absolute local paths — no file transfer needed.  All qsub calls are
    batched into one SSH round-trip to avoid per-job connection overhead.
    Job IDs returned by qsub are logged line by line.
    """
    if not job_paths:
    ...
    abs_paths = " ".join(str(p.resolve()) for p in job_paths)

    # Submit all jobs in a single SSH session to avoid per-job handshake overhead
    cmd = f"for f in {abs_paths}; do {config['PBS_QSUB_PATH']} $f; done"
...
def run_pbs_jobs(**kwargs):
    """Generate and submit PBS jobs for a boost_corr run.

    Entry point called by the CLI when --gpu-id -3 is specified.  Delegates to
    _create_pbs_scripts and _submit_pbs_scripts in order.  The submit server
    shares the same filesystem, so no file transfer step is needed.
