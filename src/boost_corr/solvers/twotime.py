...
def compute_diagonal_average(
    index: torch.Tensor, weights: torch.Tensor
) -> torch.Tensor:
...
class TwotimeCorrelator:
    ...
    def compute_average(self, num_part: int = 10) -> None:
        ...
        self.pixel_sum = torch.zeros(
            self.pixel_num, dtype=torch.float32, device=self.device
        )
    ...
    def get_saxs(self) -> dict:
        ...
        saxs = {
            "saxs2d": self.pixel_sum.reshape(-1)[self.mask_crop],
            "saxs2d_par": self.pixel_sum_par.reshape(num_par, -1)[:, self.mask_crop],
