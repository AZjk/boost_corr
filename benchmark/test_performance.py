"""Module for performance testing of boost_corr benchmarks.
This module contains tests for performance metrics evaluating host and GPU performance.
"""
...
def test(queue_size=128):
    """Test performance for a given queue size on the default device.

    Parameters:
        queue_size (int, optional): The size of the queue for performance testing. Default is 128.
    """
    ...
    xb = MultitauCorrelator(
        det_size=det_size, frame_num=frame_num, queue_size=512, device="cuda:1"
    )
    # xb.debug()
...
def test_host_gpu(queue_size=1):
    """Test GPU performance for a given queue size.

    Parameters:
        queue_size (int, optional): The size of the queue for GPU performance testing. Default is 1.
    """
    batch_size = queue_size
    # det_size = (1024, 512)
    # det_size = (516, 1556)
